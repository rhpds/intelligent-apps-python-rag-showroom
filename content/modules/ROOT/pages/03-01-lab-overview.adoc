...
5.  Copy your API token into the `openai_api_key` variable, replace the `EMPTY` string:
...
6.  *Execute the cells* in the notebook one by one. The key cells perform the following actions:
