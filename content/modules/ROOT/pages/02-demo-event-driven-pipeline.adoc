...
Try uploading to your per-user bucket (e.g., `{user}-pdf-inbox`) or the shared `pdf-inbox` bucket:
...
Run the following command to see the event appear in the topic:
...
You will see the raw JSON event from MinIO appear on the screen, confirming it has been successfully brokered by Kafka.
...
. **Knative KafkaSource:** This component is continuously polling the `minio-events` Kafka topic. You can view the logs of the event processing pod to see events flowing through.
+
[source,bash]
----
# Get the event bridge pod in the workshop namespace
oc get pods -n rag-pipeline-workshop -l app=minio-event-bridge

# View the logs to see events being processed
oc logs -n rag-pipeline-workshop -l app=minio-event-bridge -f
----
+
...
2.  Inside your Data Science Project (e.g., `{user}`), click on **Pipelines**.
...
* A PDF uploaded to the shared `pdf-inbox` triggers a run in the default namespace.
