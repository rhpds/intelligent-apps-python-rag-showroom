...
2.  **Knative Broker & Trigger:** The Broker receives the CloudEvent (including the `bucketname` extension) and delivers it to any matching subscribers. Our `kfp-s3-trigger` service is subscribed via a Trigger. We can verify the Trigger is ready and pointing to the correct service.
    ```bash
